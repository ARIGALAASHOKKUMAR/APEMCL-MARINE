    ...
    try {
      ...
      if (res.status === 200) {
        ...
        const qrCodesWithImages = generatedQRs.map((qr, index) => ({
          ...qr,
          qrImageUrl:
            res.data.qrCodes?.[index]?.qrImageUrl ||
    ...
    try {
      ...
      if (downloadResult.status === 200) {
        ...
        if (Platform.OS === "android") {
          ToastAndroid.show(
            `QR Code ${index + 1} saved to gallery`,
            ToastAndroid.SHORT,
          );
        } else {
  ...
  const downloadAllQRCodes = async () => {
    ...
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Permission Denied",
          "Cannot save QR codes without storage permission",
        );
        return;
      }
      ...
      if (Platform.OS === "android") {
        ToastAndroid.show(
          `${downloaded} QR Codes saved to gallery`,
          ToastAndroid.LONG,
        );
      }
    } catch (error) {
              ...
              {qrCodes.length > 0 && (
                <View style={styles.statsContainer}>
                  ...
                  <Text style={styles.statsSubText}>
                    Generated at:{" "}
                    {new Date(qrCodes[0]?.timestamp).toLocaleString()}
                  </Text>
                </View>
              )}
